...
if [ ${new_uav} != ${merg_uav} ]; then
  ...
  rosbag play $rosbag_path /${orig_uav}/hector_mapping/map_throttle:=/${merg_uav}/hector_mapping/received_map${map_num} ${renames} /${orig_uav}/odometry/odom_main:=/${merg_uav}/odometry/odom_main_rec${map_num} ${renames} -l -s 220 -u 10
else
  rosbag play $rosbag_path ${renames} -l -s 220 -u 10
fi
